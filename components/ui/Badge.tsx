...
const variants = {
  purple: 'bg-[--flx-purple]/12 border-[--flx-purple]/25 text-violet-300',
  cyan:   'bg-[--flx-cyan]/10 border-[--flx-cyan]/20 text-cyan-300',
  ...
  new:    'bg-[--flx-cyan]/12 border-[--flx-cyan]/30 text-cyan-200',
...
export function Badge({ variant = 'purple', className, children, ...props }: BadgeProps) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1.5 px-2 py-0.5 rounded border backdrop-blur-md select-none',
        'text-[9px] font-bebas tracking-[1.5px] uppercase transition-all duration-300',
        variants[variant],
        className
      )}
      {...props}
