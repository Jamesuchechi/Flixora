...
export default async function MoviesPage() {
  ...
  const featured = popular.results.slice(0, 1)[0];
  const grid     = popular.results.slice(1, 19);
  ...
  return (
    <div className="min-h-screen px-10 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-['Bebas_Neue',sans-serif] text-4xl tracking-[3px] text-[--flx-text-1]">Movies</h1>
          <p className="text-sm text-[--flx-text-3] mt-1">{popular.total_results.toLocaleString()} titles available</p>
        </div>
      </div>

      {/* Featured banner */}
      {featured && (
        <Link href={`/movies/${featured.id}`} className="block relative h-[280px] rounded-2xl overflow-hidden mb-10 group">
          <Image
          ...
          <div className="absolute bottom-8 left-8">
            <p className="text-[10px] text-[--flx-cyan] tracking-widest uppercase font-semibold mb-2">Featured</p>
            <h2 className="font-['Bebas_Neue',sans-serif] text-4xl tracking-wide mb-2">{featured.title}</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {grid.map((movie) => (
          <Link key={movie.id} href={`/movies/${movie.id}`} className="group">
            ...
            <p className="text-[12px] font-medium text-[--flx-text-1] truncate">{movie.title}</p>
