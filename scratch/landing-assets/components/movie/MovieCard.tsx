...
export function MovieCard({
  ...
  return (
    <div className={cn('flex-shrink-0 w-[140px] group cursor-pointer', className)}>
      <Link href={href}>
        <div className="relative w-[140px] h-[210px] rounded-xl overflow-hidden mb-2.5 transition-transform duration-200 group-hover:-translate-y-1">
          ...
          {/* Rank ghost number */}
          {rank && (
            <span className="absolute top-2 left-2.5 font-['Bebas_Neue',sans-serif] text-3xl leading-none text-white/10 select-none">
              {rank}
            </span>
          )}
