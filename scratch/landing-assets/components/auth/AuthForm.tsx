...
export function AuthForm({ mode, redirectError }: AuthFormProps) {
  ...
  useEffect(() => {
    if (state.success && mode === 'signup') { /* show email confirmation */ }
  }, [state, mode]);
  ...
  const inputClass = 'w-full bg-[--flx-surface-2] border border-[--flx-border] hover:border-white/15 focus:border-[--flx-purple]/50 rounded-lg px-4 py-2.5 text-sm text-[--flx-text-1] placeholder-[--flx-text-3] outline-none transition-colors';

  return (
    <div className="bg-[--flx-surface-1] border border-[--flx-border-p] rounded-2xl p-8 shadow-2xl w-full">
