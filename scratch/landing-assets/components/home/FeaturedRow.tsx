  ...
  return (
    <Link href={`/series/${show.id}`} className="flex-shrink-0 relative w-[290px] h-[165px] rounded-xl overflow-hidden group cursor-pointer block">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3.5">
        <p className="font-['Bebas_Neue',sans-serif] text-[17px] tracking-wide mb-0.5">{show.name}</p>
  ...
  return (
    <section className="px-10 py-7">
      <div className="flex items-center justify-between mb-5">
        <h2 className="font-['Bebas_Neue',sans-serif] text-xl tracking-[2px] text-[--flx-text-1] flex items-center gap-2.5">
          {title}
          <Badge variant="new">NEW</Badge>
        </h2>
